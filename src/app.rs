...
pub fn App() -> impl IntoView {
    ...
    view! {
        ...
        <Stylesheet id="leptos" href="/pkg/headermeup.css"/>
...
fn HomePage() -> impl IntoView {
    #[cfg(feature = "ssr")]
    {
        println!("About to expect the context");
        // this can be done inline because it's synchronous
        // if it were async, we'd use a server function
        let resp = expect_context::<leptos_spin::ResponseOptions>();
        resp.append_header("X-Wizz", "spork".as_bytes());
    }
...
pub async fn save_count(count: u32) -> Result<(), ServerFnError> {
    ...
    store.set_json("headermeup_count", &count).map_err(|e| ServerFnError::ServerError(e.to_string()))?;
    Ok(())
}
